...
class SemiCarousel extends React.Component {
  ...
  animateArticleChange = (index, length) => {

    var currentindex = this.state.index;

    if (currentindex !== index && index > 0 && index < length) {
        ...
        return(<>
          ...
          <div>
                ...
                <div id="text-carousel" className="content bloc animate__animated animate__fadeInRight">
                  ...
                  <Row>
                    <Col className="centerArrow" xl={2} md={2} xs={2}> 
                      <Button onClick={()=>this.animateArticleChange(this.state.index -1, newslength)} className="fontStyle next_news">
            ...
            <div className='miniatureMargin'>
              {news.map((article) => (
                <a onClick={()=>this.animateArticleChange(article.id, newslength)}>
                  <img src={article.miniature} className='miniature'/>
                </a>
              ))}
            </div>
          </div>
